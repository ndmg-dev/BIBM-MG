...
export default function EvolutionChart({ data }: EvolutionChartProps) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    // When shared={false}, payload contains only the item currently being hovered natively by Recharts
      ...
      <div className="flex-1 w-full min-h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line yAxisId="right" type="monotone" dataKey="margem_bruta" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" dot={{r: 4, fill: '#3b82f6', strokeWidth: 0}} activeDot={{r: 6}} name="Margem Bruta %" />
            <Line yAxisId="right" type="monotone" dataKey="margem_liquida" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" dot={{r: 4, fill: '#8b5cf6', strokeWidth: 0}} activeDot={{r: 6}} name="Margem Liq. %" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
